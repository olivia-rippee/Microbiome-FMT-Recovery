...
RETENTION_FILE = Path("results/phase10/"
    "denoising-stats-export/"
    "stats.tsv")

ALPHA_FILE = Path("results/phase12/"
    "alpha-diversity-summary.tsv")

BETA_FILE = Path("results/phase12/"
    "distance-from-baseline.tsv")

ALPHA_STATS_FILE = Path("results/phase13/tables/"
    "alpha_pairwise_statistics.tsv")

BETA_STATS_FILE = Path("results/phase13/tables/"
    "beta_baseline_distance_statistics.tsv")

PERSISTENCE_FILE = Path("results/phase14/tables/"
    "genus_ancombc_persistence.tsv")

TOP_GENERA_FILE = Path("results/phase15/tables/"
    "top_persistent_genera.tsv")

TIMEPOINTS = ["before", "7d", "14d", "30d"]

POST_TIMEPOINTS = ["7d", "14d", "30d"]
...
for path in [RETENTION_FILE, ALPHA_FILE, BETA_FILE, ALPHA_STATS_FILE,
    BETA_STATS_FILE, PERSISTENCE_FILE, TOP_GENERA_FILE]:
    require_file(path)
...
retention = pd.read_csv(RETENTION_FILE, sep="\t", comment="#")

alpha = pd.read_csv(ALPHA_FILE, sep="\t")

beta = pd.read_csv(BETA_FILE, sep="\t")

alpha_stats = pd.read_csv(ALPHA_STATS_FILE, sep="\t")

beta_stats = pd.read_csv(BETA_STATS_FILE, sep="\t")

persistence = pd.read_csv(PERSISTENCE_FILE, sep="\t")

top_genera = pd.read_csv(TOP_GENERA_FILE, sep="\t")
...
participants = (alpha["participant"].nunique())

samples = len(alpha)

retention_col = ("percentage of input non-chimeric")

median_retention = (retention[retention_col].median())

min_retention = (retention[retention_col].min())

max_retention = (retention[retention_col].max())
...
print("Median non-chimeric read retention: "
    f"{median_retention:.2f}%")

print("Retention range: "
...
alpha_summary = (alpha.groupby("timepoint")[
	["observed_features", "shannon", "faith_pd",
	"evenness"]].median().reindex(TIMEPOINTS))
...
print(alpha_summary.to_string(float_format=lambda x: f"{x:.3f}"))
...
beta_summary = (beta.groupby("timepoint")[
        ["bray_curtis", "jaccard", "weighted_unifrac",
         "unweighted_unifrac"]].median().reindex(POST_TIMEPOINTS))
...
print(beta_summary.to_string(float_format=lambda x: f"{x:.3f}"))
...
persistent3["label"] = (
    persistent3["taxon"].apply(short_taxon_name))

persistent3["direction_consistent"] = (
    persistent3["directions"].apply(
        lambda value: len(set(str(value).split(";"))) == 1))

n_persistent = len(persistent3)

n_persistent_increased = (persistent3[
        persistent3["directions"]
        .str.contains("increased")].shape[0])

n_persistent_decreased = (persistent3[
        persistent3["directions"]
        .str.contains("decreased")].shape[0])
...
print("Genera significant at all three "
    f"post-FMT timepoints: {n_persistent}")

print(f"Persistent increases: {n_persistent_increased}")

print(f"Persistent decreases: {n_persistent_decreased}")
...
baseline_alpha = alpha_stats[
    alpha_stats["state_1"].eq("before")].copy()

baseline_alpha_sig = (baseline_alpha[
        baseline_alpha["significant_fdr_0.05"]])

post_beta_sig = (beta_stats[
        beta_stats["significant_fdr_0.05"]])
...
print("Significant baseline-vs-post alpha contrasts: "
    f"{len(baseline_alpha_sig)}/"
    f"{len(baseline_alpha)}")

print("Significant post-FMT beta-distance contrasts: "
    f"{len(post_beta_sig)}/"
    f"{len(beta_stats)}")
...
add_result("cohort", "participants", "all", participants,
    "Complete four-timepoint recipient cohort")

add_result("cohort", "samples", "all", samples,
    "52 participants × 4 timepoints")

add_result("sequencing_qc", "median_non_chimeric_retention_percent",
    "all", round(median_retention, 3),
    "Median DADA2 non-chimeric read retention")


for timepoint in TIMEPOINTS:

    row = alpha_summary.loc[timepoint]

    for metric in ["observed_features", "shannon", "faith_pd", "evenness",]:

        add_result("alpha_diversity", metric, timepoint,
	round(row[metric], 6), "Median across participants")


for timepoint in POST_TIMEPOINTS:

    row = beta_summary.loc[timepoint]

    for metric in ["bray_curtis", "jaccard", "weighted_unifrac",
        "unweighted_unifrac"]:

        add_result("beta_diversity", metric, timepoint,
            round(row[metric], 6),
            "Median distance from participant-specific pre-FMT baseline")


add_result("differential_abundance", "persistent_significant_genera",
    "7d_14d_30d", n_persistent,
    "Significant at all three post-FMT timepoints")

add_result("differential_abundance", "persistent_increased_genera",
    "7d_14d_30d", n_persistent_increased,
    "Significant persistent increases")

add_result("differential_abundance", "persistent_decreased_genera",
    "7d_14d_30d", n_persistent_decreased, "Significant persistent decreases")


key_results = pd.DataFrame(records)

key_results.to_csv(TABLES / "cohort_key_results.tsv", sep="\t", index=False)
...
ax.hist(retention[retention_col], bins=20)

ax.axvline(median_retention, linewidth=2)

ax.set_xlabel("Non-chimeric read retention (%)")

ax.set_ylabel("Number of samples")

ax.set_title("Cohort DADA2 read retention")

fig.tight_layout()

fig.savefig(FIGURES / "sequencing_retention_summary.png",
...
metrics = [("shannon", "Shannon"), ("faith_pd", "Faith PD")]

for metric, label in metrics:

    values = (alpha_summary[metric].values)

    ax.plot(x, values, marker="o", label=label)

ax.set_xticks(x)

ax.set_xticklabels(["Before", "7d", "14d", "30d"])

ax.set_xlabel("FMT timepoint")

ax.set_ylabel("Median diversity")

ax.set_title("Alpha diversity following FMT")

ax.legend()

fig.tight_layout()

fig.savefig(FIGURES / "alpha_diversity_summary.png",
    dpi=300, bbox_inches="tight")
...
x = np.arange(len(POST_TIMEPOINTS))

for metric, label in [("bray_curtis", "Bray-Curtis"),
    ("weighted_unifrac", "Weighted UniFrac")]:

    values = (beta_summary[metric].values)
...
ax.set_ylabel("Median distance from pre-FMT baseline")

ax.set_title("Persistent community displacement following FMT")

ax.legend()

fig.tight_layout()

fig.savefig(FIGURES / "beta_distance_from_baseline.png",
    dpi=300, bbox_inches="tight")
...
plot_genera = (top_genera.copy())

if "label" not in plot_genera.columns:
    plot_genera["label"] = (plot_genera["taxon"]
        .apply(short_taxon_name))

plot_genera = (plot_genera.sort_values("mean_lfc"))

fig, ax = plt.subplots(figsize=(10, 8))

ax.barh(plot_genera["label"], plot_genera["mean_lfc"])
...
ax.set_title("Strongest persistent differential genera")

fig.tight_layout()

fig.savefig(FIGURES / "top_persistent_genera.png",
    dpi=300, bbox_inches="tight")
...
summary_lines = ["Integrated longitudinal FMT cohort summary", "",
...
fig.savefig(FIGURES / "integrated_cohort_summary.png",
    dpi=300, bbox_inches="tight")
...
print("Median DADA2 non-chimeric retention: "
    f"{median_retention:.2f}%")
...
print("Median Bray-Curtis distance from baseline:")
...
print("Median weighted UniFrac distance from baseline:")
...
print("Persistent significant genera: "
    f"{n_persistent}")

print("Persistent increased genera: "
    f"{n_persistent_increased}")

print("Persistent decreased genera: "
    f"{n_persistent_decreased}")
...
print("""The cohort-level analysis was integrated across sequencing
