...
genus["mean_relative_abundance"] = (
    genus[TIMEPOINT_ORDER].mean(axis=1)
)
...
genus_ranked.to_csv(
    TABLE_DIR / "genus_relative_abundance_ranked.tsv",
    sep="\t",
    index=False,
)
...
top_output.to_csv(
    TABLE_DIR / "top_genera_relative_abundance.tsv",
    sep="\t",
    index=False,
)
...
changes[
    ...
    TABLE_DIR / "genus_baseline_changes.tsv",
    sep="\t",
    index=False,
)
...
phylum_output = phylum[
    ["Phylum"] + TIMEPOINT_ORDER
].copy()

phylum_output.to_csv(
    TABLE_DIR / "phylum_relative_abundance.tsv",
    sep="\t",
    index=False,
)
...
plot_df = (
    phylum
    .set_index("Phylum")[TIMEPOINT_ORDER]
    .T
)
...
plt.savefig(
    FIGURE_DIR / "phylum_relative_abundance.png",
    dpi=300,
)
...
genus_plot = (
    genus_plot
    .groupby("Plot_group")[TIMEPOINT_ORDER]
    .sum()
    .T
)
...
ax.set_xlabel("FMT timepoint")
ax.set_title(
    f"Top {TOP_N_GENERA} Genera Across the FMT Time Course"
)
...
plt.savefig(
    FIGURE_DIR / "top_genera_relative_abundance.png",
    dpi=300,
)
...
trajectory = (
    top
    .set_index("Genus")[TIMEPOINT_ORDER]
)
...
plt.savefig(
    FIGURE_DIR / "top_genera_trajectories.png",
    dpi=300,
)
...
fig, ax = plt.subplots(
    figsize=(7, max(5, TOP_N_GENERA * 0.45))
)
...
plt.savefig(
    FIGURE_DIR / "top_genera_heatmap.png",
    dpi=300,
)
...
print(
    """
Longitudinal taxonomic composition was summarized across
